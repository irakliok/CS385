...
run_test_with_args_and_input() {
    ...
    (timeout --preserve-status "$maxtime" "$command" $args &> "$outputfile"; echo $? > "$statusfile") &> /dev/null << EOF
$input
EOF
    end=$(date +%s.%N)
    ...
    case $status in
        ...
        *)
            ...
            if [ "$computed_output" = "$expected_output" ]; then
                ...
                # Why 93?  Why not 93!
                valgrind --leak-check=full --error-exitcode=93 $command $args &> /dev/null << EOF
$input
EOF
                vgstatus=$?
