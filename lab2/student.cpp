 ...
 * Date          : 1/29/22
 * Description   : User enters information for multiple students, prints students with GPA < 1.0
...
vector<Student>  find_failing_student(const vector<Student> &students) {
    ...
    for (const auto &student: students) {
        if (student.gpa() < 1) { failing_students.push_back(student); }
    }
    return failing_students;
}
void print_students(const vector<Student> &students){
    for (const auto &student: students) {student.print_info();}
}
int main() {
